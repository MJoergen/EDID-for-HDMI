...
BEGIN
    ...
    BEGIN
        IF RISING_EDGE(clk) THEN
            ...
            WHEN START => IF clockDiv(6 DOWNTO 5) = "11" THEN
                currentState <= DONE;
            END IF;
            WHEN STOP => IF clockDiv(6 DOWNTO 5) = "11" THEN
                currentState <= DONE;
            END IF;
            WHEN READ => IF clockDiv = "1111111" THEN
                IF bitSend = 7 THEN
                    currentState <= SEND;
                END IF;
            END IF;
            WHEN WRITE => IF clockDiv = "1111111" THEN
                IF bitSend = 7 THEN
                    currentState <= RCV;
                END IF;
            END IF;
            ...
            WHEN DONE => IF NOT enable THEN
                currentState <= IDLE;
            END IF;
            WHEN SEND => IF clockDiv = "1111111" THEN
                currentState <= DONE;
            END IF;
            WHEN RCV => IF clockDiv = "1111111" THEN
                currentState <= DONE;
            END IF;
            END CASE;
        END IF;
